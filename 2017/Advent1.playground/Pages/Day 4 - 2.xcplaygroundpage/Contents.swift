...
func valid(_ p: Substring) -> Bool {
    var words: Set<Substring> = []
    for word in p.split(separator: " ") {
        let (inserted, _) = words.insert(word)
        if !inserted { return false }
    }
    
    for (a, b) in words.pairs() where a.count == b.count {
